...
export const displayGedit = (closeWindow, hideWindow, isActive) => {
    // If we have Window parameters, use them, otherwise just render the Gedit component
    if (closeWindow !== undefined && hideWindow !== undefined && isActive !== undefined) {
        return (
            <Window
                ...
                isActive={isActive}
                title="Contact Me"
                appIcon="./themes/Yaru/apps/gedit.png"
            >
                <Gedit />
            </Window>
        );
    } else {
        // This is for when it's called from a terminal command or similar
        return <Gedit />;
    }
}
